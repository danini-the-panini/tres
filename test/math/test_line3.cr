...
class TestLine3 < Minitest::Test
  ...
  def test_constructor_equals
    ...
    a = Tres::Line3.new(two3.clone, one3.clone)
    assert_equal(two3, a.start_point)
    assert_equal(one3, a.end_point)
  end

  def test_copy_equals
    a = Tres::Line3.new(zero3.clone, one3.clone)
    b = Tres::Line3.new.copy(a)
  ...
  def test_at
    a = Tres::Line3.new(one3.clone, Tres::Vector3.new(1, 1, 2))

    assert_in_delta(a.at(-1).distance_to(Tres::Vector3.new(1, 1, 0)), 0.0001)
    assert(a.at(0).distance_to(one3.clone) < 0.0001)
    assert_in_delta(a.at(1).distance_to(Tres::Vector3.new(1, 1, 2)), 0.0001)
    assert_in_delta(a.at(2).distance_to(Tres::Vector3.new(1, 1, 3)), 0.0001)
  end
  ...
  def test_closest_point_to_point_closest_point_to_point_parameter
    ...
    assert_in_delta(b1.distance_to(Tres::Vector3.new(1, 1, 1)), 0.0001)

    # nearby the ray
    assert_equal(-1, a.closest_point_to_point_parameter(zero3.clone, false))
    b2 = a.closest_point_to_point(zero3.clone, false)
    assert_in_delta(b2.distance_to(Tres::Vector3.new(1, 1, 0)), 0.0001)

    # nearby the ray
    assert_equal(1, a.closest_point_to_point_parameter(Tres::Vector3.new(1, 1, 5), true))
    b = a.closest_point_to_point(Tres::Vector3.new(1, 1, 5), true)
    assert_in_delta(b.distance_to(Tres::Vector3.new(1, 1, 2)), 0.0001)
