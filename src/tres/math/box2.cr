...
module Tres
  class Box2
    ...
    def initialize(min = nil, max = nil)
      @min = min || Tres::Vector2.new(Float32::INFINITY, Float32::INFINITY)
      @max = max || Tres::Vector2.new(-Float32::INFINITY, -Float32::INFINITY)
    end
    ...
    def center(target = Tres::Vector2.new)
      target.add_vectors(@min, @max).multiply_scalar(0.5)
    end

    def size(target = Tres::Vector2.new)
    ...
    def parameter(point, target = Tres::Vector2.new)
    ...
    def clamp_point(point, target = nil)
      result = target || Tres::Vector2.new
