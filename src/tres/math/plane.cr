...
module Tres
  class Plane
    ...
    def project_point(point, target = Tres::Vector3.new)
      self.ortho_point(point, target).sub(point).negate
    end

    def ortho_point(point, target = Tres::Vector3.new)
    ...
    def intersect_line(line, target = Tres::Vector3.new)
      v1 = Tres::Vector3.new
    ...
    def coplanar_point(target = Tres::Vector3.new)
      target.copy(@normal).multiply_scalar(- @constant)
    end

    def apply_matrix4(matrix, normal_matrix = Tres::Matrix3.new.normal_matrix(matrix))
      v1 = Tres::Vector3.new
      v2 = Tres::Vector3.new
